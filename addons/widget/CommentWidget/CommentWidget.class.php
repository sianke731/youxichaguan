...
class CommentWidget extends Widget {
	...
	public function render($data) {
		...
		$var ['userinfo'] = model ('User')->getUserInfo($this->mid);
		...
		if ($var ['showlist'] == 1) { // 默认只取出前10条
			...
			if (! empty ( $map ['row_id'] )) {
				...
				$var ['count'] = $var ['list']['data']?count($var ['list']['data']):0;
			}
		} // 渲染模版
	...
	public function parseComment($data) {
		...
		return $this->renderFile ( dirname ( __FILE__ ) . "/_parseComment_yx.html", $data ); //JTee 2014-01-19 换模板
	}
