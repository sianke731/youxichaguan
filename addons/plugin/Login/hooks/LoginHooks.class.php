...
class LoginHooks extends Hooks {
    ...
    public function login_input_footer($param) {
        ...
        if ($regInfo ['register_type'] != 'admin' && ! empty ( $platform )) {
            ...
            $html .= '</div>';
            echo $html;*/
			//JTee 2014-02-09 修改HTML结构
        	$html = '<div  class="oh mt5">';
            foreach ( $platform as $key => $value ) {
                if($key == 'sina'){
                	$html .= '<a href="'.$value.'" class="reg-btn reg-btn1" style="margin-left:66px;"></a>';
                }
                if($key == 'qzone'){
                	$html .= '<a href="'.$value.'" class="reg-btn reg-btn2"></a>';
                }
            }
        	$html .= '</div>';
        	echo $html;
