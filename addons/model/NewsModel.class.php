	...
	public function getList($limit = 20, $type= 0 , $keywords = '',$order = '',$findPage = true)
	{
	    ...
	    foreach ($data as $k => $v)
	    {
            ...
            if ($v['image'])
            {
                ...
                if ($attach)
                {
                    $thumb = getImageUrl($attach['save_path']. $attach['save_name'],335,180);   
                }
            }
            $data[$k]['image'] = $thumb ;
            $data[$k]['title_intro'] = msubstr($v['news_title'],0,28);
            $data[$k]['content_intro'] = msubstr(strip_tags($v['news_content']),0,180);
            //获取评论数量 
