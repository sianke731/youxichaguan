...
class UserModel extends Model {
	...
	public function getUserInfo($uid) {
		...
		$user['score'] = model('Credit')->getUserCredit($uid);  //JTee 2014-01-19 获取积分
		$user['score'] = $user['score']['credit']['score']['value'];
		$user['bind'] = M ('login')->where("uid='{$uid}'")->select(); //JTee 2014-01-19 同步帐号
		if($user['bind']){
			foreach($user['bind'] as $key=>$val){
				$bind[$val['type']] = $val;
			}
			$user['bind'] = $bind;
		}
		//print_r($user['bind']);die;
