...
技术  QQ：2355373292
修改时间：2013-08-18
程序功能：创明网PHP接口示例 通过接口进行动态参数短信接口；
修改日期:	2013-08-12
说明:		http://smsapi.c123.cn/OpenPlatform/OpenApi?action=sendParam&ac=用户账号&authkey=认证密钥&cgid=通道组编号&c=短信内容&m=发送号码&t=发送时间&p1=动态参数&p2=动态参数
状态:
	1 操作成功
...
$authkey = '13EEC8C89B5875613F14E92B37188888';		//认证密钥
$cgid='11'; //通道组编号
$c = '';		//内容
$m= '15102110086,18156012707';	//号码
$csid='4';   //签名编号 ,可以为空时，使用系统默认的编号
$t='{p1}你好，你在{p2}吧';      //发送时间,可以为空表示立即发送
$p1='小明{|}小王';  //p为动态参数,多个值使用”{|}”分隔,动态参数的值数量应该与发送号码数一致,并且p小写，每条动态短信允许包含10个和以内的动态参数
$p2='上海{|}北京';
...
function sendSMS($url,$ac,$authkey,$cgid,$m,$c,$csid,$t,$p1,$p2)
{
	$data = array
		(
		'action'=>'sendParam',  //发送类型 ，可以有sendOnce短信发送，sendBatch一对一发送，sendParam	动态参数短信接口
		'ac'=>$ac,					  //用户账号
		'authkey'=>$authkey,	     //认证密钥
		'cgid'=>$cgid,              //通道组编号
		'm'=>$m,		     //号码,多个号码用逗号隔开
		'c'=>iconv('gbk','utf-8',$c),		    //如果页面是gbk编码，则转成utf-8编码，如果是页面是utf-8编码，则不需要转码,内容用{|}，如测试一{|}测试二
		'csid'=>$csid,            //签名编号 ，可以为空，为空时使用系统默认的签名编号
		't'=>$t,                      //定时发送，为空时表示立即发送,yyyyMMddHHmmss 如:20130721182038
		'p1'=>iconv('gbk','utf-8',$p1),  //如p1为小明{|}小王
		'p2'=>iconv('gbk','utf-8',$p2)   //如p2为上海{|}北京
		);
