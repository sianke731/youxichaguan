...
class EventModel extends BaseModel{
	...
	public function getEventcount($map=''){
	...
	public function appendContent( $data ){
		...
		$data['city']        = $opts_list['city'];  //JTee 2014-1-27
  ...
  //           $data['content'] = t(getEventShort($eventMap['explain'],40));
  //           X('Notify')->send($fids,'event_add',$data, $eventMap['uid']);
	...
	public function getEventContent( $eventId,$uid){
		...
		//TODO 活动议程
		$result['times_list'] = M("event_times")->where(array("event_id"=>$eventId))->select();
		
		
		
		
		//TODO 活动嘉宾
		$result['guest_list'] = M("event_guest")->where(array("event_id"=>$eventId))->select();
		foreach ($result['guest_list'] as $k =>$v){
			$result['guest_list'][$k]["avatar"] = model("attach")->getAttachByIds($v["avatar"]);
		}
	...
	public function doEditData( $time,$id ){
		...
		if( $this->where( 'id='.$id )->setField( 'eTime',$time ) ){
			return 1;
		}else{
			return 0;
		}
	...
	/*
	//获取我参加的活动
	public function get_my_active($map){
		$list = $this->event->join
	...
	*/
	...
	{
		$model   =   D('Event_guest');
		$ret = $model->where('event_id='.$id)->select();
		return $ret ;
	}
	...
	{
		$model   =   D('Event');
		$ret = $model->where('id='.$id)->save(array('is_verify'=>3));
		return $ret ;
	}
	 ...
	 * @return mixed
	 */
	public function setGuest($event_id)
	{
	 ...
	 * @return mixed
	 */
	public function setTimes($event_id)
	{
		$ret = array( 'ret' => false, 'msg' => '更新信息失败' );
		$model   =   D('Event_times');
		if($model->create())
