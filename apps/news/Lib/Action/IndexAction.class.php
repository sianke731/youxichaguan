...
class IndexAction extends Action 
{
	...
	public function index()
	{
	    ...
	    //热门点评
	    $hotComment = D('Comment')->where("`table`='news' AND is_del=0 AND is_audit=1")->order('comment_id DESC')->limit(5)->select();
	    foreach($hotComment as &$vo){
	    ...
	    $this->assign('newsList',$this->_news->getList($limit,$real_tid,$key));
	    $this->assign('topList',$this->_news->getList(3,0,'','is_top desc,news_id desc',false));  //JTee 2014-01-15 推荐新闻
	    $this->assign('hotList',$this->_news->getList(6,0,'','hits desc',false));  //JTee 2014-01-15 推荐新闻
