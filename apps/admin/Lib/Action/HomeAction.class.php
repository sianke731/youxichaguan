...
class HomeAction extends AdministratorAction {
	...
	{
		//统计待审核通讯录
		$verifyInfo['verifyMember'] = D('Member')->where('is_verify=0')->count();
		//统计待认证通讯录
		$verifyInfo['authMember'] = D('Member')->where('is_auth=0')->count();
		//统计待审核产品
		$verifyInfo['verifyGame'] = D('Game')->where('is_verify=0')->count();
		//统计待认证产品
		$verifyInfo['authGame'] = D('Game')->where('is_auth=0')->count();
		//统计待审核公司
		$verifyInfo['verifyCompany'] = D('Company')->where('is_verify=0')->count();
		//统计待认证公司
		$verifyInfo['authCompany'] = D('Company')->where('is_auth=0')->count();
		//统计待审核活动
		$verifyInfo['verifyEvent'] = D('Event')->where('is_verify=0')->count();
