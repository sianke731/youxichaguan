...
class RegisterAction extends Action
{
	...
	public function doStep1(){	
		...
		if($uid) {
			...
			if($invite) {
				...
				//绑定通讯录
				$invite = model('Invite')->where("code=".$inviteCode)->find();
				if(!empty($invite['member_id'])){
					$member = M("member")->where("id=".$invite['member_id'])->find();
					if(empty($member['uid'])){
						M("member")->where("id=".$invite['member_id'])->setField("uid",$uid);
					}
				}
	...
	public function waitForActivation() {
		...
		if($user_info) {
			if($user_info['is_audit'] == '0') {
				...
				// 激活
				//exit(U('public/Register/step2',array(),true));	  //JTee 2014-02-09 屏蔽头像上传			
			}
		} else {
			// 注册
			$this->redirect('public/Passport/login');
	...
	public function activate() {
		...
		if ($active) {
			...
			$this->assign('messages','恭喜您注册成功！');
			$this->display(); //JTee 2014-01-19 修改注册步骤
		} else {
			//$this->redirect('public/Passport/login');
			//$this->error($this->_register_model->getLastError());
			$this->assign('messages',$this->_register_model->getLastError());
			$this->display(); //JTee 2014-01-19 修改注册步骤
		}
	}
