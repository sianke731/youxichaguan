...
class IndexAction extends Action {
	...
	public function index(){
		//新闻栏目
		$newsCategory = model('CategoryTree')->setTable('news_category')->getNetworkList() ;
		$typeNames = array();
		foreach($newsCategory as $k=>$c){
			$typeNames[$c['id']] = $c['title'];
		}
		$this->assign('typeNames' , $typeNames);
		
		//新闻
		$newsList = D('news')->field('type_id,news_id,news_title,news_content,uid,image,hits,created')->where('state=1 and is_del=0')->order('news_id DESC')->findPage(6);
		foreach($newsList['data'] as &$v) {
			$v['type_ids'] = explode(',',$v['type_id']);
			$v['date'] = date('m月d日',$v['created']);
		}
		$this->assign('newsList' , $newsList);
		
		//热门文章
		$hotNews = D('news')->field('news_id,news_title')->where('state=1 and is_del=0 and is_top=1')->order('news_id DESC')->limit(10)->select();
		$this->assign('hotNews' , $hotNews);
		
		//投稿文章
		$contributeNews = D('news')->field('news_id,news_title')->where('state=1 and is_del=0')->order('news_id DESC')->limit(14)->select();
		$this->assign('contributeNews' , $contributeNews);
		
		//最新产品
		$newGame = D('Game')->field('id,name,logo,tags,platform,targets')->where('is_verify=3')->order('id DESC')->limit(4)->select();
		$this->assign('newGame' , $newGame);
		
		//最新公司
		$newCompany = D('Company')->field('id,name,logo')->where('is_verify=3')->order('id DESC')->limit(4)->select();
		$this->assign('newCompany' , $newCompany);
		
		//最新名片
		$newMember = M('Member a')->JOIN(C('DB_PREFIX').'company b ON a.company_id=b.id')
					->field('a.id,a.name,a.avatar,a.company_id,b.name as company_name')->where('a.is_verify=3')->order('a.id DESC')->limit(12)->select();
		$this->assign('newMember' , $newMember);
		
		//最新活动
		$newEvent = D('Event')->field('id,title,sTime,coverId,address,uid')->where('is_verify=3')->order('id DESC')->limit(5)->select();
		$this->assign('newEvent' , $newEvent);
		
		//最新评论
		$newComment = D('Comment')->field('comment_id,uid,content,ctime')->where('is_del=0 AND is_audit=1')->order('comment_id DESC')->limit(4)->select();
		foreach($newComment as &$vo){
			$vo['userinfo'] = model('User')->getUserInfo($vo['uid']);
		}
	...
	public function addsuggest(){
		...
		$data['message'] = t($_POST['message']);
		$data['uid'] = $this->mid;
		$data['ctime'] = time();
		$result = D('Suggest')->add($data);
		if($result){
			$this->success('感谢你的参与，我们会尽快处理！');
		}else{
			$this->error('发生错误！');
		}
	}
